...
class VsphereClient():
    ...
    def get_brief_performance(self, moid, histlevel, perftype):
        perfResults = self.query_performance(moid, histlevel, perftype)
        if perfResults:
            metric = perfResults[0]
            # print metric
            sampletime = []
            starttime = str(
                metric.sampleInfo[0].timestamp.replace(tzinfo=pytz.utc).astimezone(pytz.timezone('Asia/Shanghai')))[
                        0:19]
            endtime = str(
                metric.sampleInfo[-1].timestamp.replace(tzinfo=pytz.utc).astimezone(pytz.timezone('Asia/Shanghai')))[
                      0:19]
            ret = {}
            ret['moid'] = moid
    ...
    def get_performance(self, moid, histlevel, perftype):
        perfResults = self.query_performance(moid, histlevel, perftype)
        if perfResults:
            metric = perfResults[0]
            # print metric
    ...
    def query_performance(self, moid, histlevel, perftype):
        try:
            si = connect.Connect(self.vcenterip, self.vcenterport, self.vcenteruser, self.vcenterpass)
            ...
            if perftype == 'vm':
                ...
                query = vim.PerformanceManager.QuerySpec(intervalId=intervalId, entity=vm,
                                                         metricId=[micpu, midisk1, midisk2, mimem, minet],
                                                         endTime=endTime)
            elif perftype == 'host':
